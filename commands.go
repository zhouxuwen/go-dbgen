...
//GenerateEntry 生成golang表对应的结构实体
func (c *commands) GenerateEntry(args []string) int {
	...
	err := c.l.CreateModelsEntity(formats)
	if err != nil {
		log.Println("GenerateEntryErr>>", err.Error())
	}
	go Gofmt(GetExeRootDir())
	return 0
}

// CustomFormat 还可以自定义结构体解析实体,如json,gorm,xml
func (c *commands) CustomFormat(args []string) int {
	formats = c._setFormat()
	return 0
}

// GenerateCURD 生成golang操作mysql的CRUD增删改查语句
func (c *commands) GenerateCURD(args []string) int {
	formats = c._setFormat()
	err := c.l.CreateGormCURD(formats)
	if err != nil {
...
// _showTableList 显示所有名视图
func (c *commands) _showTableList(NameAndComment []TableNameAndComment) {
	for _, table := range NameAndComment {
		info := fmt.Sprintf("%s:%s", strconv.Itoa(table.Index), table.Name)
		if table.Comment != "" {
...
func (c *commands) _setFormat() []string {
	fmt.Print("Set the mapping name of the structure, separated by a comma (example :json,gorm) default:json,gorm>")
	...
	defaultFormList := make([]string, 0)
	defaultFormList = append(defaultFormList, "json")
	defaultFormList = append(defaultFormList, "gorm")
