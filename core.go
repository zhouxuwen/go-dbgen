...
func start() {
	...
	app.Action = func(c *cli.Context) error {
		...
		if c.NumFlags() > 0 {
			...
			if tableName == "" {
				return cli.NewExitError("table name is null, please use -t params", 9)
			}
			...
			if err := Commands(tableName); err != nil {
...
func usage() {
	...
	app.Usage = "Quickly generate CURD and documentation for operating PostgresSQL.etc" //说明
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{Name: "t", Usage: "table name, all is all tables"},
		&cli.BoolFlag{Name: "debug", Usage: "debug", Value: false},
	}
}

// Commands 实现命令功能
func Commands(tableName string) error {
	var err error
	Conn, err = InitPostgresDB(DbConn)
	if Conn == nil || err != nil {
	...
	err = logic.DB.GetTableNameAndComment(tableName)
	if err != nil {
		return err
	}
	fmt.Printf(" start generate >>>\n")
